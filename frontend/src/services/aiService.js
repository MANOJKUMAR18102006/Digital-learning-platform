...
const explainConcept = async (documentId, concept) => {
    try {
        const response = await axiosInstance.post(API_PATHS.EXPLAIN_CONCEPT, { documentId, concept });
        return response.data;
    } catch (error) {
...
const getChatHistory = async (documentId) => {
    try {
        const response = await axiosInstance.get(API_PATHS.GET_CHAT_HISTORY(documentId));
        return response.data;
    } catch (error) {
